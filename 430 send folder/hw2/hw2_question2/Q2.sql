...
--select  f."from", count(p.pid), sum(t.price), avg(f.miles)
...
select f."from", count(p.pid), sum(t.price), temp.avgmiles
from (select avg(f1.miles) as avgmiles, f1."from" as departairport
      from flights f1
      group by f1."from")temp, tickets t, flights f, passengers p
where t.pid = p.pid and t.fid = f.fid and f."from" = temp.departairport
group by f."from", temp.avgmiles;
