...
int ttyread(int dev, char *buf, int nchar)
{
  ...
  int event = devtab[dev].dvnum == TTY0? TTY0_INPUT: TTY1_INPUT;
  ...
  while (i < nchar) { /* loop until we get user-specified # of chars */
    ...
    if (queuecount( &(tty->rbuf) )) /* if there is something in rbuf */
      ...
      sleep(event, FREAD);	/* added for hw5--don't spin here either */
...
int ttywrite(int dev, char *buf, int nchar)
{
  ...
  int i = 0;
  int saved_eflags;
  int event = devtab[dev].dvnum == TTY0? TTY0_OUTPUT: TTY1_OUTPUT;
  ...
  /* if nec., now tell transmitter to interrupt (or restart output) */
  if ((inpt(baseport+UART_IER)& UART_IER_THRI) == 0) {
      outpt( baseport+UART_IER, UART_IER_RDI | UART_IER_THRI); 
  }

  set_eflags(saved_eflags);

  /* START OF HW3 CHANGES: wait instead of spin
...
/* hw5: USE_SECONDARY_METHOD is not implemented */
...
void irqinthandc(int dev)
{
  ...
  if ((lsr = inpt(baseport+UART_LSR)) & UART_LSR_DR) { /* if it's read ready*/
    ...
    event = devtab[dev].dvnum == TTY0? TTY0_INPUT: TTY1_INPUT;
    wakeup(event);
    if (tty->echoflag){	/* if echoing wanted */
      enqueue(&tty->ebuf,ch);	/* echo char */
      if (queuecount( &tty->ebuf )==1)  /* if first char...*/
  ...
  if (lsr & UART_LSR_THRE) { /* if it's tx ready */
    event = devtab[dev].dvnum == TTY0? TTY0_OUTPUT: TTY1_OUTPUT;
    ...
    if (queuecount( &tty->ebuf )) { /* if there is char in echo Q output it*/
      ...
      /* HW3 change: wakeup processes waiting for this event */
      wakeup(event);		/* TTY0_OUTPUT or TTY1_OUTPUT */
